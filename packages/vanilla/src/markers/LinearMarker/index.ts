...
export class LinearMarker extends BaseMarker implements LinearBound {
  ...
  /** Getter & Setter */
  public getLineLength = (x1: number, y1: number, x2: number, y2: number) => {
  ...
  protected resize(
    ...
    if (this.activeGrip) {
      if (
        ...
        this.getLineLength(this.x1 + x, this.y1 + 1, this.x2, this.y2) >=
          this.MIN_LENGTH
      ) {
        this.x1 += x;
        this.y1 += y;
        ...
        this.getLineLength(this.x1, this.y1, this.x2 + x, this.y2 + y) >=
          this.MIN_LENGTH
      ) {
        this.x2 += x;
        this.y2 += y;
  ...
  private addControlBox = () => {
    this.controlBox = SvgHelper.createGroup([
      ['class', 'fc-whiteboard-line-control-box'],
    ]);
  ...
  private createGrip = (): ResizeGrip => {
    ...
    grip.visual.addEventListener('touchstart', this.onTouch, {
      passive: false,
    });
