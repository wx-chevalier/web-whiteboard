...
import { Mode, Source } from './../../utils/types';
import './index.css';
...
export class WhitePage {
  ...
  constructor(
    ...
    if (whiteboard) {
      this.mountContainer =
        whiteboard.drawboardMountPoint === 'body'
          ? document.body
          : this.whiteboard!.target;
    }
  }
  ...
  public applySnap(snap: WhitepageSnap) {
    ...
    snap.markers.forEach(markerSnap => {
      ...
      if (marker) {
        ...
        const newMarker = this.drawboard.addMarker(
          getMarkerByType(markerSnap.type),
          {
            id: markerSnap.id,
          },
        );
  ...
  private initSource(source: Source) {
    ...
    if (typeof source.imgSrc === 'string' && !this.parentContainer) {
      throw new Error(
        'Invalid source, If you set image url, you must also set parentContainer',
      );
    }
